...
| 항목 | v1.0 | v1.1~v1.2 변경 내용 | 근거 |
|---|---|---|---|
| 병렬 캡처 & 사이트 확대 | Mock 대체 | **정부24, Q-Net, 국민건강보험, OPIC, TOEIC 5개 사이트 자동화 추가.** 바이브 코딩 구동 가능한 **로컬 RPA 에이전트(Node.js Puppeteer 등) 연동** 구조로 확정 | 피드백 반영 |
| LLM 크로스체크 | 단순 텍스트 비교 | **Vision LLM**을 활용하여 원본 서류, 자동 조회 화면, 지원서 입력값을 종합적으로 교차 검증 | 피드백 반영 |
| 토익 등 로그인 세션 | 수동 조회 | **대시보드 UI에 계정 정보 입력란**을 제공하여, 로컬 RPA가 로그인 후킹 또는 자동 로그인을 수행하도록 지원 | 피드백 반영 |
...
| 아키텍처 전환 | 웹 기반 배포 (Vercel) | **로컬 퍼스트(Local-First)** 데스크톱 구동 기반 전환. 클라우드 전송 없이 로컬에서 데이터베이스(SQLite) 및 처리 | 피드백 반영 |
| 대량 파일 처리 | 단건 수동 업로드 | **로컬 폴더 경로 스캔 기반 대량 서류 자동 일괄 처리(Bulk Processing)** 도입 | 피드백 반영 |
...
4. 5대 사이트(정부24, Q-Net, 건강보험, OPIC, 토익) 감사원 제출용 병렬 캡처 리포트 **자동 생성률 100%** (로컬 RPA 활용)
5. 허위 서류 검출률 **≥ 95%** (Vision LLM 기반 원본-조회본-지원서 크로스체크)
6. **검증 결과 엑셀 내보내기**: 배치 완료 후 1클릭으로 xlsx 파일 생성·다운로드 — 담당자 사후 리뷰 워크플로우 지원
7. **파일 자동 정규화**: 비표준 파일 형식 → PDF/JPG 자동 변환율 **≥ 95%** / 이미지 회전 자동 보정
8. **로컬 퍼스트(Local-First) E2E 보안**: 모든 서류 원본 및 검증 결과(SQLite DB) 로컬 PC 보관률 **100%** (민감 개인정보 클라우드 유출 제로)
9. **로컬 폴더 대량 스캔**: 수천 명 폴더 경로 지정 시 수동 업로드 없이 자동 스캔·처리율 **100%**
...
**AC-1 | 5대 사이트 로컬 RPA 병렬 캡처 완결성**
- **Given** 지원자가 자격증/학위 서류를 시스템에 업로드하고 검증이 완료된 상태에서
- **When** 관리자가 로컬 RPA 에이전트(또는 크롬 확장프로그램)를 켜둔 상태로 검증 엔진이 사이트(정부24, Q-Net, 국민건강보험, OPIC, 토익) 조회를 지시할 때
- **Then** 로컬 RPA가 백그라운드에서 해당 사이트를 조회하고, 원본 서류 이미지 + 기관 조회 결과 화면이 단일 프레임 내에 나란히 캡처되어 Vercel 서버로 전송, 저장되어야 한다.
  - 임계치: 캡처 성공률 **≥ 99.5%** (배치당 n ≥ 100건 기준) / 타임스탬프 정밀도 **± 1초 이내**
  - 바이브 코딩 제약 대응: 클라우드 Browserless.io 대신 로컬 머신의 Puppeteer를 활용하여 개발 복잡도와 서버 비용을 낮춤.

**AC-2 | 로그인 필요 사이트(토익 등) 접근 우회 (UI 지원)**
- **Given** TOEIC 등 로그인 세션이 필수적인 사이트 조회가 필요한 상태에서
- **When** 담당자가 대시보드 UI의 '계정 설정' 란에 해당 사이트의 로그인 정보(ID/Password)를 입력해두면
- **Then** 로컬 RPA 에이전트가 이를 전달받아 백그라운드 브라우저에서 자동으로 로그인을 수행(또는 세션 후킹)하여 조회 후 캡처를 완수해야 한다.
...
**AC-1 | Vision LLM 기반 크로스체크(Cross-check) 완결성**
- **Given** 지원자가 제출한 '입사지원서 내용', '업로드한 서류 이미지', 그리고 '로컬 RPA가 캡처한 기관 조회 화면'이 모두 확보된 상태에서
- **When** Gemini Vision LLM 엔진이 이 세 가지 데이터를 종합 분석할 때
- **Then** 발급번호, 성명, 취득일자 등 모든 데이터 포인트가 세 자료에서 일치하는지 비교하고, 1개 이상 불일치 시 '불일치' 플래그 및 상세 불일치 항목, 그리고 LLM의 판정 사유가 명시되어야 한다.
  - 임계치: 검증 정확도(Precision) **≥ 95%** / False Negative(위조 서류 미검출) 율 **< 1%**
  - 파이썬 스크립트에 의존하는 텍스트 매칭 대신, LLM이 시각적 맥락과 내용의 '충실도'를 포괄적으로 검수하도록 구현.
...
### Story-5 | 로컬 퍼스트 기반 대규모 서류 일괄 스캔 (Bulk Processing)

**Story:**
> As a **공공기관 채용 PM (김철수)**, I want **내 PC의 최상위 지원자 폴더 경로(C:\Applicants)만 지정하면 시스템이 하위 폴더의 수천 개 서류를 자동 스캔하여 일괄 처리**받고 싶다, so that **민감한 지원자 개인정보가 포함된 파일을 외부 클라우드 서버에 업로드하지 않고 내부망(내 PC)에서 안전하게 검증**을 마칠 수 있다.

**AC-1 | 채용공고 및 지원자 구조화 폴더 경로 자동 인식 (v1.3 변경)**
- **Given** 로컬 퍼스트 환경(데스크톱 구동)에서 대시보드 화면에 접속한 상태에서
- **When** 담당자가 특정 채용공고의 최상위 폴더 경로(예: `C:\JobPostings\2026_공채`)를 입력하고 '일괄 스캔 시작'을 클릭하면
- **Then** 시스템이 하위의 `수험번호_이름` 폴더들을 식별하고, 각 지원자 폴더 내의 지정된 하위 구조(`1. 입사지원서`, `2. 자격증`, `3. 경력증명서`, `4. 어학사항`)를 스캔하여 검증 큐에 자동 등록해야 한다.
  - 브라우저를 통한 파일 '업로드' 과정 불필요. 로컬 Node.js 백엔드가 파일 시스템(fs)에 직접 접근.

**AC-2 | 외부 클라우드 전송 원천 차단 및 결과물 로컬 생성 (v1.3 변경)**
- **Given** 일괄 스캔 및 검증이 완료된 상태에서
- **When** 검증 결과 처리를 수행할 때
- **Then** 원본 파일, RPA 캡처 이미지, 검증 결과(DB)는 모두 구동 중인 로컬 PC에만 저장되어야 하며, 특히 각 지원자 폴더 내에 **`5. 진위확인결과`** 폴더를 자동 생성하여 결과물(PDF, 엑셀, 캡처 이미지 등)을 식별 가능한 파일명으로 저장해야 한다.
  - 단, E2E 크로스체크를 위해 Gemini API로만 일회성 암호화 전송되며, API 응답 완료 후 즉시 파기됨을 기술적으로 보장한다.
...
graph TD
    ...
    M2["M2. 5대 사이트 로컬 RPA 병렬 캡처<br>(정부24, Q-Net, 건강보험, OPIC, TOEIC)<br>+ 토익 계정 입력 UI"]
    M3["M3. Vision LLM 크로스체크<br>(원본 서류 vs 조회 화면 vs 입사지원서)"]
    M4["M4. 파일 형식 변환 및 이미지 보정"]
    M5["M5. 감사원 제출용 PDF 리포트 자동 생성"]
    M6["M6. 엑셀(.xlsx) 검증 결과 내보내기"]
    M7["M7. 파일 자동 명명 규칙"]
    M8["M8. 채용공고 폴더 대량 스캔 및<br>1. 입사지원서 기준 대조"]
    M9["M9. 다중 페이지 파일 분할(Split) 및<br>진위확인결과 로컬 폴더 저장"]
    end
    ...
    S4["S4. 엑셀 내보내기 고도화<br>(지원서 vs 실제결과 비교)"]
    end
...
| 기능 | 우선순위 | 기존 대안 | 차별 근거 |
|---|---|---|---|
| **M2. 5대 사이트 로컬 RPA 병렬 캡처** | **Must** | 수기 스크린샷 | Vercel의 환경 제약을 우회하기 위한 **로컬 RPA** 전략. 토익 등 로그인 필요 사이트는 계정 입력 UI로 극복. 자동화율 100% |
| **M3. Vision LLM 크로스체크** | **Must** | 단순 코드(파이썬) 텍스트 비교 | LLM의 포괄적 인식 능력으로 허위 검출률 극대화, 맥락 기반 판단 |
...
- **비표준 파일 형식(DOCX, HEIC, BMP, TIFF) 변환 및 다중 페이지 분리(Split) 지원** *(v1.3 신규)*
- **이미지 회전 자동 보정 + 다중 페이지 순서 변경/추출 UI** *(v1.3 신규)*
- 정부24, Q-Net, 국민건강보험, OPIC, TOEIC 5대 사이트 로컬 RPA 연동 기반 병렬 캡처 자동화 (바이브 코딩 최적화 아키텍처)
- TOEIC 등 로그인 필수 사이트 우회를 위한 대시보드 내 계정(ID/PW) 입력 UI 제공
- Gemini Vision LLM을 활용한 **'1. 입사지원서' 추출 데이터 vs 증빙 서류 vs 로컬 RPA 결과 간의 포괄적 크로스체크 로직** *(v1.3 구조화)*
- 감사원 제출용 PDF 리포트 자동 생성
- **엑셀(.xlsx) 검증 결과 데이터 내보내기 (지원서 내용 vs 증빙서류 결과 비교)** *(v1.3 고도화)*
...
- **웹 서버 배포 생략 및 로컬 퍼스트 환경(데스크톱 구동) 기반 개인정보 보호 극대화**
- **채용공고 기준의 구조화된 폴더(1~4번) 자동 스캔 및 검증 후 `5. 진위확인결과` 폴더 결과물 자동 생성** *(v1.3 신규)*
...
- 모바일 앱 (데스크톱 로컬 구동 전용 MVP)
- 채용 플랫폼 독자 구축 (API First 전략 유지)
- 외부 API 연동 (Should/Could 항목 → Phase 2)
- **클라우드 서비스(Vercel/Supabase) 배포 및 연동 (로컬 퍼스트 전환으로 명시적 제외)**
