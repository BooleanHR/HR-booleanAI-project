...
**Tech Stack:** Next.js (App Router) · 로컬 SQLite (Prisma) · 로컬 File System · Gemini API (Vercel AI SDK) · Tailwind CSS · shadcn/ui · Local Node.js 구동 · exceljs · sharp  
...
| 1.1 | 2026-04-18 | User Review | **사용자 검토 의견 5건 반영**: ① 병렬 캡처 MVP 대안 ② 엑셀 내보내기 ③ 선택적 안내 ④ 파일 변환 ⑤ 자동 명명. |
| **1.2** | **2026-04-25** | **User Review** | **개인정보 보호 극대화를 위한 로컬 퍼스트(Local-First) 전환**: Vercel/Supabase 배포 취소, 로컬 SQLite 및 File System 사용. **로컬 폴더 대량 일괄 스캔(Bulk Scan)** 도입. |
...
>
> **v0.3 대비 추가된 것 (v1.1~v1.2):**
> - 5대 사이트(정부24, Q-Net, 건강보험, OPIC, 토익) **로컬 RPA 에이전트** 기반 병렬 캡처 (바이브코딩 최적화 아키텍처)
> - Vision LLM을 활용한 서류-조회화면-지원서 **크로스체크(Cross-check)** 검증
> - 토익 등 로그인 세션 우회를 위한 대시보드 내 **계정 설정 UI** 도입
...
> - **로컬 퍼스트 아키텍처**: 클라우드 전송 없이 내 PC 안에서 검증 완결 (SQLite DB + File System)
> - **대량 폴더 일괄 스캔**: 수천 명 지원자 폴더 경로를 입력하면 자동으로 스캔 및 처리
>
...
| **5대 사이트 원본-조회화면 병렬 캡처가 실행 및 저장된다** | **로컬 RPA가 로그인 및 조회 후 캡처한 이미지가 DB에 저장됨을 확인** | **F3 (병렬 캡처)** |
| **Vision LLM 기반 입사지원서 크로스체크가 수행된다** | **원본, 캡처본, 지원서 텍스트 간 불일치를 LLM이 판단하여 반환함 확인** | **F4 (LLM 크로스체크)** |
...
| **로컬 폴더를 대량으로 자동 스캔 및 처리한다** *(v1.2)* | **최상위 로컬 경로 입력 → 하위 지원자 폴더 단위 자동 큐 등록 확인** | **F13 (로컬 폴더 대량 스캔)** |
...
| Mock 대체 항목 | 이유 | 대체 방식 |
|---|---|---|
| HRDK API 등 E2E 완전 자동화 (예외 케이스) | 비정상 접속 지연, 점검 시간 등 | 타임아웃 시 Mock JSON 응답 반환으로 수동 큐 이관 시뮬레이션 |
| 카카오 알림톡 실발송 | 발신번호 사전 등록 + 템플릿 심사 2주 소요 | `console.log` + 대시보드 알림 텍스트 표시 |
| Vercel 내장 Browserless.io | Vercel Serverless 실행 시간 초과 및 설정 복잡 | **관리자 PC에서 구동되는 로컬 Node.js Puppeteer 스크립트(로컬 RPA 에이전트)로 우회 구현하여 실물 캡처 달성** |
...
| **C-TEC-003** | DB는 **Prisma + 로컬 SQLite** 만 사용 *(v1.2 변경)* | 개인정보 보호를 위해 클라우드(Supabase) 사용을 배제. 로컬 `.db` 파일로 보관 |
...
| **C-TEC-007** | 배포 인프라 없이 **Local Node.js (데스크톱 앱 형태)** 구동 *(v1.2 변경)* | Vercel 배포를 취소하고 개인정보 로컬 처리. `npm run dev/start` 로 구동 |
...
| IS-02 | 5대 사이트 진위확인 연동 (토익 로그인 포함) | 로컬 RPA 에이전트(Node.js Puppeteer) 연동 |
| IS-03 | Vision LLM 기반 E2E 크로스체크 | Gemini Vision API E2E 검증 |
| IS-04 | Audit Trail DB 기록 + 원본-조회본 병렬 캡처 저장 | Prisma (로컬 SQLite) + 로컬 File System |
...
| **OS-11** | **웹 기반 클라우드 배포 (Vercel/Supabase)** *(v1.2 명시적 제외)* | **개인정보 보호 리스크로 로컬 퍼스트 아키텍처 고정** |
...
| ADR-002 | **Gemini Vision** — OCR 및 크로스체크 전용 모델 | 무료 할당량 충분; 시각적 맥락 종합 판단에 유리 |
| ADR-003 | **로컬 RPA 에이전트 도입** (바이브코딩 최적화) | Vercel에서 Headless Browser 불가를 로컬 PC 실행으로 우회 |
...
| **ADR-008** | **병렬 캡처 구조** — 로컬 RPA를 통한 실제 화면 캡처 저장 | 감사 증빙력 완벽 확보 (API 스냅샷에서 상향) |
| **ADR-008-B** | **로그인 세션 우회** — 대시보드 내 토익 계정 입력 UI | 토익 등 인증 필수 사이트 접속 자동화 목적 |
...
graph LR
    ...
    subgraph NodeEnv["Local Node.js Environment (로컬 퍼스트)"]
        SA["Server Actions"]
        RH["Route Handlers"]
    ...
    subgraph LocalSystem["로컬 PC 자원"]
        PG["SQLite (Prisma)"]
        ST["로컬 File System (원본+변환 파일)"]
        AUTH["단일 세션/Mock Auth"]
    end
...
| Must | F2 | 5대 사이트 진위확인 로컬 RPA | Route Handler → 로컬 RPA 에이전트 호출 |
| Must | F3 | Audit Trail DB 기록 + **로컬 RPA 캡처 저장** | Prisma → Supabase PostgreSQL |
| Must | F4 | Vision LLM 크로스체크 | Gemini Vision API 종합 판단 |
...
datasource db {
  // 로컬 개발: SQLite (v1.2 로컬 퍼스트: 배포 시에도 SQLite 사용)
  provider = "sqlite"
  url      = "file:./dev.db"
}
...
sequenceDiagram
    ...
    participant ST as 로컬 File System

    User->>UI: 단건 업로드 또는 대규모 폴더 스캔 지정
    UI->>SA: ingestDocuments(localPath) 또는 uploadAndVerify(formData) 호출
    ...
    alt OCR 신뢰도 < 70 (PoC 임계치)
        ...
        Note over SA: 로컬 RPA 에이전트 호출 (정부24, 토익 등)
        SA->>GOV: 진위확인 조회 (토익의 경우 대시보드 설정된 ID/PW 전달)
        alt RPA 성공
            GOV-->>SA: { verified: true/false, capture_image_url: "..." }
            Note over SA: verification_method = "로컬 RPA 캡처"
        else RPA 타임아웃
            SA->>SA: 수동 큐 전환
            Note over SA: mock_used = false (실제 동작)
        end

        Note over SA: Vision LLM 크로스체크 (E2E)
        SA->>GEM: 원본 이미지 + RPA 캡처 이미지 + 지원서 입력값 전송
        GEM-->>SA: { match: true/false, discrepancy: "..." }

        Note over SA: Triple Check Layer 3 — 최종 판정
        SA->>SA: finalJudgment(layer1, layer2) → PASS | FAIL
        Note over SA: 다중 검증 완료 기준 적용<br/>LLM 내용 일치 + 공적 식별자 확인 → final_result="완료"<br/>그 외 → final_result="확인필요"

        SA->>DB: VERIFICATION_JOB 저장 (status, discrepancy_detail, confidence_score, verification_method, final_result)

        Note over SA: 실제 RPA 병렬 캡처 이미지 저장
        SA->>DB: AUDIT_TRAIL 생성 (timestamp_hash, is_immutable=true, capture_file_path, 로컬_rpa_캡처_path)
...
graph TB
    ...
    subgraph NodeEnv["Local Node.js (로컬 구동)"]
        SA["Server Actions\n(비즈니스 로직)"]
        RH["Route Handlers\n(API 엔드포인트)"]
        MW["Middleware\n(세션 확인)"]
        AI["Vercel AI SDK\n(Gemini Vision 호출)"]
    ...
    subgraph LocalResource["로컬 PC 자원"]
        PG["SQLite DB\n(Prisma ORM)"]
        ST["로컬 File System\n(원본+변환 파일)"]
        AUTH["단일 세션 로직"]
    end
...
### F1 — 대량 로컬 폴더 스캔 및 단건 추출 엔진 (Must)

| ID | 요구사항 | Priority | AC |
|---|---|---|---|
| **REQ-FUNC-001** | 시스템은 대시보드에서 채용공고 폴더 경로(예: `C:\JobPostings\2026_공채`)를 입력받아 하위 지원자 폴더 구조(`수험번호_이름`)를 스캔하고, 내부의 `1. 입사지원서, 2. 자격증, 3. 경력증명서, 4. 어학사항` 폴더를 자동 매핑하여 검증 큐에 등록해야 한다. *(v1.3 구조화)* | **Must** | **Given** 채용공고 경로 입력 **When** 스캔 시작 **Then** 1~4번 폴더 파일 탐색 및 자동 큐 등록 완료 |
...
| ID | 요구사항 | Priority | AC |
|---|---|---|---|
| REQ-FUNC-010 | Triple Check Loop는 Server Action에서 순차 실행한다: (1) **'1. 입사지원서' 파싱 JSON** vs 증빙서류 OCR값 비교, (2) 증빙서류 OCR값 vs 공공 API 조회값 비교, (3) 최종 판정. 각 단계 결과는 VERIFICATION_JOB에 기록. *(v1.3 기준 변경)* | Must | **Given** 파싱/OCR 완료 **When** Triple Check 실행 **Then** 입사지원서 데이터를 기준(Input)으로 대조 실행 |
...
| REQ-FUNC-014 | 상세 화면은 상단에 '원본 서류'와 '로컬 RPA 캡처 이미지'를 병렬 뷰어로 보여주고, 하단에 입사지원서/OCR/기관조회 3단 비교 테이블 및 AI 판정 사유를 표시해야 한다. *(v1.3 변경)* | Must | **Given** 검증 건 존재 **When** 상세 화면 클릭 **Then** 상하단 분리된 병렬 캡처 및 대조 테이블 표시 |
...
| **REQ-FUNC-025** | **검증이 완료된 건에 대해, 시스템은 지원자의 원본 로컬 경로 하위에 `5. 진위확인결과` 폴더를 자동 생성하고 생성된 감사 리포트 PDF 및 RPA 캡처 화면을 직관적인 파일명(예: `[완료]_홍길동_리포트.pdf`)으로 물리 저장해야 한다 (`fs.writeFileSync`).** *(v1.3 신규)* | **Must** | **Given** 검증 최종 승인 완료 **When** 결과 저장 로직 실행 **Then** 로컬 파일시스템의 해당 지원자 폴더 내에 5번 폴더와 결과 파일 생성 확인 |
...
| **REQ-FUNC-085** | **다중 페이지 PDF 또는 복수 이미지인 경우, 썸네일 UI를 통해 특정 페이지만 선택하여 분리(Split) 또는 제외시킬 수 있어야 한다.** *(v1.3 신규)* | **Should** | **Given** 5페이지 PDF **When** 2페이지만 선택 후 자르기 **Then** 2페이지만 검증 파이프라인으로 전달 |
...
**Document ID:** SRS-001 | **Revision:** 1.2 | **Date:** 2026-04-25 | **Standard:** ISO/IEC/IEEE 29148:2018  
**Tech Stack:** Next.js (App Router) · 로컬 SQLite (Prisma) · 로컬 File System · Gemini API (Vercel AI SDK) · Tailwind CSS · shadcn/ui · pdf-lib · exceljs · sharp · Local Node.js  
